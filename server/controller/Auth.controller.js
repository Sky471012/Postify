const getAccessToken = async (code) => {
  const body = new URLSearchParams({
    ...
    redirect_uri: `${process.env.Backend_URL}/api/linkedin/callback`,
  });
...
const linkedinCallback = async (req, res) => {
  try {
    ...
    return res.redirect(
      `${process.env.Frontend_URL}/linkedin-redirect?name=${encodeURIComponent(name)}&picture=${encodeURIComponent(picture)}`
    );
  } catch (error) {
    res.status(500).json({
